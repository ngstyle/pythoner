...
# 加载底图 88*128
base_img = Image.open('/Users/chon/Desktop/UI/11/majiang/CardBack_0_0.png')
# 可以查看图片的size和mode，常见mode有RGB和RGBA，RGBA比RGB多了Alpha透明度
# print base_img.size, base_img.mode
box = (21, 38, 67, 112)  # 底图上需要P掉的区域
print(base_img.mode)
...
for i in range(0, 34):
    ...
    if i <= 8:
        base_img.save('./handmah_{}.png'.format(i+21))  # 保存图片
    elif i <= 17:
        base_img.save('./handmah_{}.png'.format(i+22))  # 保存图片
    elif i <= 26:
        base_img.save('./handmah_{}.png'.format(i-7))   # 保存图片
    elif i <= 33:
        base_img.save('./handmah_{}.png'.format(i + 14))  # 保存图片